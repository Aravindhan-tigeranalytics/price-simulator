...
export class UnitModel{
    ...
    // newObject : NewUnit;
    constructor(
      category,product_group,retailer,year,
      date,lpi_percent,rsp_increase_percent,cogs_increase_percent,base_price_elasticity,
      cross_elasticity,net_elasticity,competition,base_units,list_price,retailer_median_base_price,
      retailer_median_base_price_w_o_vat,on_inv_percent,off_inv_percent,tpr_percent,gmac_percent_lsv,
      product_group_weight_in_grams,mars_on_invoice,mars_nrv,tpr_budget,mars_net_invoice_price,
    ...
    this.total_rsv_w_o_vat = parseFloat(this.base_units.replace(/,/g, '')) * parseFloat(this.retailer_median_base_price_w_o_vat)
...
export class NewUnit extends UnitModel{
  ...
  constructor( category,product_group,retailer,year,
    date,lpi_percent,rsp_increase_percent,cogs_increase_percent,base_price_elasticity,
    cross_elasticity,net_elasticity,competition,base_units,list_price,retailer_median_base_price,
    retailer_median_base_price_w_o_vat,on_inv_percent,off_inv_percent,tpr_percent,gmac_percent_lsv,
    product_group_weight_in_grams,mars_on_invoice,mars_nrv,tpr_budget,mars_net_invoice_price,
  ...
  updateValues(){
    this.suggested_retailer_median_base_price_w_o_vat = parseFloat(this.retailer_median_base_price_w_o_vat) * (1 + parseFloat(this.rsp_increase_percent))
    this.new_base_units = parseFloat(this.base_units.replace(/,/g, '')) * ( 1 + (parseFloat(this.base_price_elasticity) *
     ((this.suggested_retailer_median_base_price_w_o_vat - parseFloat(this.retailer_median_base_price_w_o_vat.replace(/,/g, '')))/
     parseFloat(this.retailer_median_base_price_w_o_vat.replace(/,/g, '')))))
    // parseFloat(this.total_rsv_w_o_vat) * parseFloat(this.trade_expense);
    this.suggested_list_price = parseFloat(this.list_price) * (1 + parseFloat(this.lpi_percent));
    ...
    this.product_group_weight_in_grams_new = parseFloat(this.product_group_weight_in_grams);
    this.mars_on_invoice_new=(this.on_inv_percent_new/100) * this.suggested_list_price;
    ...
    this.mars_net_invoice_price_new = this.mars_nrv_new - (this.mars_nrv_new * this.tpr_percent_new)
    this.mars_off_invoice_new = (this.off_inv_percent_new / 100) * this.mars_net_invoice_price_new
    this.mars_nsv_new = this.mars_net_invoice_price_new - this.mars_off_invoice_new
    this.retailer_mark_up_new = ((this.suggested_retailer_median_base_price_w_o_vat/this.mars_nsv_new) - 1) * 100
    this.mars_cogs_per_unit_new = parseFloat(this.mars_cogs_per_unit) * (1 + parseFloat(this.cogs_increase_percent))
    this.total_rsv_w_o_vat_new = this.new_base_units * this.suggested_retailer_median_base_price_w_o_vat
    this.total_lsv_new = this.new_base_units * this.suggested_list_price
    this.mars_total_on_invoice_new = (this.on_inv_percent_new / 100) * this.total_lsv_new
    this.mars_total_nrv_new = this.total_lsv_new - this.mars_total_on_invoice_new
    this.tpr_budget_new_nrv =  this.tpr_percent_new * this.mars_total_nrv_new
    this.mars_total_net_invoice_price_new = this.mars_total_nrv_new - this.tpr_budget_new_nrv
    this.mars_total_off_invoice_new = (this.mars_total_net_invoice_price_new * this.off_inv_percent_new) / 100
    this.total_nsv_new = this.total_lsv_new - this.mars_total_on_invoice_new - this.tpr_budget_new_nrv - this.mars_total_off_invoice_new
    this.total_cogs_new =  this.mars_cogs_per_unit_new * this.new_base_units
    this.total_weight_in_tons_new = (this.product_group_weight_in_grams_new * this.new_base_units)/1000000;
    this.trade_expense_new = this.mars_total_on_invoice_new+ this.tpr_budget_new_nrv+this.mars_total_off_invoice_new
    this.retailer_margin_new = this.total_rsv_w_o_vat_new - this.total_nsv_new
    this.retailer_margin_percent_of_rsp_new = (this.retailer_margin_new / this.total_rsv_w_o_vat_new) * 100
    this.mars_mac_new = this.total_nsv_new - this.total_cogs_new
    this.mars_mac_percent_of_nsv_new = (this.mars_mac_new / this.total_nsv_new) * 100
    this.mars_percent_of_total_profit_pool_new = (this.mars_mac_new / (this.mars_mac_new + this.retailer_margin_new)) * 100
    this.retailer_percent_of_total_profit_pool_new = (this.retailer_margin_new / (this.retailer_margin_new + this.mars_mac_new)) * 100
    this.rsv_w_o_vat_$_chg = this.total_rsv_w_o_vat_new -  (this.total_rsv_w_o_vat )
    this.nsv_$_chg = this.total_nsv_new - parseFloat(this.total_nsv.replace(/,/g, ''))
    this.cogs_$_chg = this.total_cogs_new - parseFloat(this.total_cogs.replace(/,/g, ''))
    this.trade_expense_$_chg = this.trade_expense_new -  parseFloat(this.trade_expense.replace(/,/g, ''))
    this.retailer_margin_$_chg = this.retailer_margin_new -  parseFloat(this.retailer_margin.replace(/,/g, ''))
    this.mars_mac_$_chg = this.mars_mac_new - parseFloat(this.mars_mac.replace(/,/g, ''))
    this.percent_rsv_w_o_vat_$_chg = ((this.total_rsv_w_o_vat_new -  (this.total_rsv_w_o_vat ))/ (this.total_rsv_w_o_vat)) * 100
    this.percent_nsv_$_chg = ((this.total_nsv_new - parseFloat(this.total_nsv.replace(/,/g, ''))) /  parseFloat(this.total_nsv.replace(/,/g, ''))) * 100
    this.percent_cogs_$_chg = ((this.total_cogs_new - parseFloat(this.total_cogs.replace(/,/g, ''))) /  parseFloat(this.total_cogs.replace(/,/g, ''))) * 100
    this.percent_trade_expense_$_chg =  ((this.trade_expense_new - parseFloat(this.trade_expense.replace(/,/g, ''))) /  parseFloat(this.trade_expense.replace(/,/g, ''))) * 100
    this.percent_retailer_margin_$_chg = ((this.retailer_margin_new - parseFloat(this.retailer_margin.replace(/,/g, ''))) /  parseFloat(this.retailer_margin.replace(/,/g, ''))) * 100
    this.percent_mars_mac_$_chg = ((this.mars_mac_new - parseFloat(this.mars_mac.replace(/,/g, ''))) /  parseFloat(this.mars_mac.replace(/,/g, ''))) * 100
